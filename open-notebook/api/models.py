...
class CreateAgentInsightRequest(BaseModel):
    """Direct insight creation, e.g. from an external agent service."""

    source_id: str = Field(..., description="Source ID to attach the insight to")
    insight_type: str = Field(..., description="Type/category of the insight")
    content: str = Field(..., description="Insight content (markdown supported)")


class AgentInsightCreationResponse(BaseModel):
    """Response for direct insight creation."""

    status: Literal["pending"] = "pending"
    message: str = "Insight creation started"
    source_id: str
    insight_type: str
    command_id: Optional[str] = None
